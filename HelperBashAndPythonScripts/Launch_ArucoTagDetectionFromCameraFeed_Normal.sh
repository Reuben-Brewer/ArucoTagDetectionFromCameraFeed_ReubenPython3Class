...
echo "Running Launch_ArucoTagDetectionFromCameraFeed_Normal.sh" | tee -a $LogFileFullPath

echo "Launch_ArucoTagDetectionFromCameraFeed_Normal.sh running GetPIDsByProcessEnglishNameAndOptionallyKill.py" | tee -a $LogFileFullPath
...
echo "Launch_ArucoTagDetectionFromCameraFeed_Normal.sh running ArucoTagDetectionFromCameraFeed.py" | tee -a $LogFileFullPath
echo "########## NOTE THAT YOU CANNOT LAUNCH A TKINTER GUI FROM /ETC/RC.LOCAL ##########" | tee -a $LogFileFullPath
sudo $PythonVirtualEnvironmentFullPath -u /home/pinis/Desktop/ArucoTagDetectionFromCameraFeed_PythonDeploymentFiles/test_program_for_ArucoTagDetectionFromCameraFeed_ReubenPython3Class.py "SOFTWARE_LAUNCH_METHOD:USERCLICKED" | tee -a $LogFileFullPath
